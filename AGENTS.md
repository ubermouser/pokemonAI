...
- To install dependencies: `conan install . --output-folder=build --build=missing -s build_type=Release -pr:b=clang -pr:h=clang`.
- To configure: `CC=clang CXX=clang++ cmake --preset conan-release`.
- To build: `CC=clang CXX=clang++ cmake --build --preset conan-release -j16`.
