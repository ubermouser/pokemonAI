...
uint32_t PlannerMax::generateSolution(
  ...
  size_t bestAction = SIZE_MAX;

  for (const auto& action: cu.getValidActions(origin, agentTeam)) {
    // produce the resulting state of iAction:
    PossibleEnvironments rEnvP = cu.updateState(
        origin,
        agentTeam==TEAM_A?action:AT_MOVE_NOTHING,
        agentTeam==TEAM_B?action:AT_MOVE_NOTHING);
  ...
  if (results != NULL) { 
      results->clear(); results->push_back(PlannerResult(1, bestAction, -1, bestFitness, bestFitness));
  }
  return (uint32_t) bestAction;
}
