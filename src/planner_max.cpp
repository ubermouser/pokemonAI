...
PlannerMax::PlannerMax(size_t agentTeam, size_t engineAccuracy)
  : Planner("max_planner-NULLEVAL", agentTeam),
  engineAccuracy_(engineAccuracy)
{
}


void PlannerMax::setEngine(std::shared_ptr<PkCU>& cu) {
  Planner::setEngine(std::make_shared<PkCU>(*cu));
  cu_->setAllowInvalidMoves();
  cu_->setAccuracy(engineAccuracy_);
}
...
uint32_t PlannerMax::generateSolution(const ConstEnvironmentPossible& origin)
{
  ...
  uint32_t iBestAction = generateSolution(*cu_, *eval_, origin, agentTeam_, &nodesEvaluated, &results);
...
uint32_t PlannerMax::generateSolution(
  ...
  for (size_t iAction = 0; iAction != AT_ITEM_USE; ++iAction)
  {
    ...
    BOOST_FOREACH(const auto& cEnvP, rEnvP)
    {
      ...
      EvalResult_t evalResult = eval.calculateFitness(cEnvP, agentTeam);
