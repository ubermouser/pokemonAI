...
TEST_F(ScreenTest, ReflectReducesPhysicalDamage) {
  ...
  auto damage_baseline = baseline_result.at(0).getEnv().getTeam(0).teammate(0).getMissingHP();
  ...
  auto damage_reflect = turn2.at(0).getEnv().getTeam(0).teammate(0).getMissingHP();
...
TEST_F(ScreenTest, LightScreenReducesSpecialDamage) {
  ...
  fpType damage_screen = turn2.at(0).getEnv().getTeam(0).teammate(0).getMissingHP();
...
TEST_F(ScreenTest, LightScreenDurationDecrements) {
  ...
  auto result = engine_->updateState(engine_->initialState(), Action::move(1), Action::wait());
  auto current_state = result.at(0);
