...
TEST_F(MoveTest, Explosion) {
  ...
  PkCU engine(cfg);
  engine.setEnvironment(environment_nv);

  auto initial_env_v = engine.initialState();
  ...
  Action actionA(Action::MOVE_0); // explosion
  Action actionB(Action::wait());

  auto possible_envs = engine.updateState(initial_env_v, actionA, actionB);
