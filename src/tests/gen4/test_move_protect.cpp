...
TEST_F(ProtectTest, BlocksStatus) {
  ...
  auto turn1 = engine_->updateState(engine_->initialState(), Action::move(0), Action::move(2));
  auto blissey = turn1.where1().teammate(0, 0);
...
TEST_F(ProtectTest, ConsecutiveUseChance) {
  ...
  auto turn2_attack = engine_->updateState(state1, Action::move(0), Action::move(0));
...
TEST_F(ProtectTest, CounterResetsOnOtherMove) {
  ...
  auto turn2 = engine_->updateState(state1, Action::move(1), Action::wait());
  ...
  auto turn3 = engine_->updateState(state2, Action::move(0), Action::move(0)); // Against Sludge Bomb
