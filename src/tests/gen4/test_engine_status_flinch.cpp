...
TEST_F(FlinchStatusTest, Test_IronHeadCausesFlinch) {
    ...
    EXPECT_TRUE(flinch_state.actor(TEAM_B, ActorProxy::ALL_TEAMMATES).isBlocked());
...
TEST_F(FlinchStatusTest, Test_FlinchWearsOff) {
    ...
    EXPECT_TRUE(flinch_state1.actor(TEAM_B, ActorProxy::ALL_TEAMMATES).isBlocked());
    ...
    EXPECT_FALSE(env2.actor(TEAM_B, ActorProxy::ALL_TEAMMATES).isBlocked());
...
TEST_F(FlinchStatusTest, Test_SlowFlinchMoveDoesNotBlock) {
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        EXPECT_FALSE(results.at(i).actor(TEAM_A, ActorProxy::ALL_TEAMMATES).isBlocked());
    }

    // Pick a state where Mew hit Snorlax
    auto hit_state = results.where1Hit(TEAM_A);
    EXPECT_TRUE(hit_state.actor(TEAM_A, ActorProxy::ALL_TEAMMATES).isHit());

    // Verify Mew can still move next turn
    auto results2 = engine_->updateState(hit_state.getEnv(), Action::move(1), Action::move(0));
    auto env2 = results2.where1();
    EXPECT_FALSE(env2.actor(TEAM_A, ActorProxy::ALL_TEAMMATES).isBlocked());
}
