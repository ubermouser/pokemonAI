...
TEST_F(OHKOTest, FailsAgainstHigherLevel) {
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    EXPECT_FALSE(results.at(i).flagsFor(TEAM_A).isHit());
  }
}
...
TEST_F(OHKOTest, AccuracyFormula) {
  ...
  FixType hitProb(0);
  for (size_t i = 0; i < results.size(); ++i) {
    if (results.at(i).flagsFor(TEAM_A).isHit()) hitProb += results.at(i).getProbability();
  }
...
TEST_F(OHKOTest, GhostImmunity) {
  ...
  for (size_t i = 0; i < res0.size(); ++i) EXPECT_FALSE(res0.at(i).flagsFor(TEAM_A).isHit());
  
  auto res1 = engine_->updateState(state, Action::move(1), Action::wait());
  for (size_t i = 0; i < res1.size(); ++i) EXPECT_FALSE(res1.at(i).flagsFor(TEAM_A).isHit());
}
...
TEST_F(OHKOTest, IdentifyBypassesGhostImmunity) {
  ...
  state.teammate(TEAM_B, 2).status().identify = 1;
...
TEST_F(OHKOTest, FlyingImmunity) {
  ...
  for (size_t i = 0; i < results.size(); ++i) EXPECT_FALSE(results.at(i).flagsFor(TEAM_A).isHit());
}
...
TEST_F(OHKOTest, OHKOEffect) {
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    if (results.at(i).flagsFor(TEAM_A).isHit()) {
