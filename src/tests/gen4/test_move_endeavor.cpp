...
TEST_F(EndeavorTest, Success) {
  ...
  env.getTeam(0).getPKV().setHP(1);

  // Target HP is full
  uint32_t targetMaxHP = env.getTeam(1).getPKV().nv().getMaxHP();
  env.getTeam(1).getPKV().setHP(targetMaxHP);

  // Turn 1: Team A uses Endeavor (Move 0)
  auto result = engine_->updateState(
      env, Action::move(0), Action::wait());

  auto target_pkv = result.where1().getTeam(1).getPKV();
  auto user_pkv = result.where1().getTeam(0).getPKV();
...
TEST_F(EndeavorTest, FailHighHP) {
  ...
  env.getTeam(0).getPKV().setHP(100);

  // Set Target HP to something lower (e.g. 50)
  env.getTeam(1).getPKV().setHP(50);
