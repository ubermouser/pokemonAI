...
class SwitcherooTest : public Gen4EngineTest {
 protected:
  void SetUp() override {
    ...
    auto team_a = TeamNonVolatile()
        .addPokemon(PokemonNonVolatile()
          .setBase(pokedex_->pokemon("hypno"))
          .addMove(pokedex_->move("switcheroo"))
          .setInitialItem(pokedex_->item("choice specs"))
          .setLevel(100))
        .addPokemon(PokemonNonVolatile()
          .setBase(pokedex_->pokemon("persian"))
          .addMove(pokedex_->move("switcheroo"))
          .setLevel(100));
...
TEST_F(SwitcherooTest, item_for_item) {
...
TEST_F(SwitcherooTest, item_behavior_propagates) {
  auto trick_item = engine_->updateState(
...
TEST_F(SwitcherooTest, no_item_for_item) {
...
TEST_F(SwitcherooTest, item_for_no_item) {
  auto team_b = TeamNonVolatile()
      .addPokemon(PokemonNonVolatile()
        .setBase(pokedex_->pokemon("blissey"))
        .addMove(pokedex_->move("softboiled"))
...
TEST_F(SwitcherooTest, sticky_hold_fails) {
  auto trick_item = engine_->updateState(
