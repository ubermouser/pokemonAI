...
TEST_F(MetalBurstTest, MetalBurstSpecial) {
  ...
  auto result = engine_->updateState(
      state_special.where1(EnvironmentBitfield().actor(1, ActorProxy::ALL_TEAMMATES).setSwitched()).getEnv(),
      Action::move(0),
      Action::move(0));
  auto state = result.where1(both_hit);
...
TEST_F(MetalBurstTest, MetalBurstFailsIfMovingFirst) {
  ...
  auto result = engine_->updateState(
      state_slow.where1(EnvironmentBitfield().actor(1, ActorProxy::ALL_TEAMMATES).setSwitched()).getEnv(),
      Action::move(0),
      Action::move(0));
