...
TEST_F(TormentTest, AppliesEffect) {
  ...
  EXPECT_TRUE(env.teammate(1, 0).status().torment);
}
...
TEST_F(TormentTest, PreventsConsecutiveMoves) {
  ...
  EXPECT_TRUE(state1.teammate(1, 0).status().torment);
  // Last action should be Seismic Toss (Move 0). iLastAction = 0 + 1 = 1.
  EXPECT_EQ(state1.teammate(1, 0).status().iLastAction, 1);
  ...
  EXPECT_EQ(state2.teammate(1, 0).status().iLastAction, 2);
...
TEST_F(TormentTest, FailsIfAlreadyTormented) {
  ...
  auto state2 = results2.where1();
  EXPECT_TRUE(state2.teammate(1, 0).status().torment);
}
...
TEST_F(TormentTest, ClearsOnSwitch) {
    ...
    EXPECT_FALSE(state2.teammate(1, 0).status().torment);
    ...
    EXPECT_FALSE(state3.teammate(1, 0).status().torment);
}
