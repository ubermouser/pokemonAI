...
TEST_F(ClearBodyTest, PreventsStatDrop) {
  auto env = setup_clearbody.at(0).getEnv();
  // Verify Metagross starts with 0 boost
  EXPECT_EQ(env.getTeam(0).getPKV().getBoost(FV_ATTACK), 0);

  auto metagross_after_growl = setup_clearbody.at(0).getEnv().getTeam(0).getPKV();
...
TEST_F(ClearBodyTest, StatDropOccursWithoutAbility) {
  auto charmander_after_growl = setup_control.at(0).getEnv().getTeam(0).getPKV();
