...
TEST_F(ScreenTest, ReflectReducesPhysicalDamage) {
  auto damage_baseline =
      mew_physdmg.where1().getEnv().getTeam(0).teammate(0).getMissingHP();
  auto damage_reflect =
      mew_reflect_physdmg.where1()
          .getEnv()
          .getTeam(0)
          .teammate(0)
          .getMissingHP();
  ...
  EXPECT_EQ(
      mew_reflect.where1().getEnv().getTeam(0).getNonVolatile().reflect, 5);
...
TEST_F(ScreenTest, LightScreenReducesSpecialDamage) {
  auto damage_baseline =
      mew_specdmg.where1().getEnv().getTeam(0).teammate(0).getMissingHP();
  auto damage_screen =
      mew_lightscreen_specdmg.where1()
          .getEnv()
          .getTeam(0)
          .teammate(0)
          .getMissingHP();
  ...
  EXPECT_EQ(
      mew_lightscreen.where1().getEnv().getTeam(0).getNonVolatile().lightScreen,
      5);
...
TEST_F(ScreenTest, ReflectDurationDecrements) {
  ...
  EXPECT_EQ(turn6.where1().getEnv().getTeam(0).getNonVolatile().reflect, 0);
}
...
TEST_F(ScreenTest, LightScreenDurationDecrements) {
  ...
  auto turn6 = engine_->updateState(current_state, Action::move(2), Action::move(0));
  current_state = result.where1();
  EXPECT_EQ(turn6.where1().getEnv().getTeam(0).getNonVolatile().lightScreen, 0);
}
...
TEST_F(ScreenTest, FailsIfAlreadyActive) {
  ...
  EXPECT_EQ(turn1.where1().getEnv().getTeam(0).getNonVolatile().reflect, 5);
  ...
  EXPECT_EQ(turn2.where1().getEnv().getTeam(0).getNonVolatile().reflect, 4);
}
