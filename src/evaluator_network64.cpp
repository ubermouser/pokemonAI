...
void evaluator_network64::resetEvaluator(const environment_nonvolatile& _envNV)
{
  envNV = &_envNV;
  // reset memoized data:
  generateBestMoves();
...
evalResult_t evaluator_network64::calculateFitness(const environment_volatile& env, size_t iTeam)
{
  return calculateFitness(*network, env, iTeam);
};

evalResult_t evaluator_network64::calculateFitness(neuralNet& cNet, const environment_volatile& env, size_t iTeam)
{
...
void evaluator_network64::seed(float* inputBegin, const environment_volatile& env, size_t _iTeam) const
{
  float* cInput;
  ...
  for (size_t iNTeam = 0; iNTeam < 2; ++iNTeam)
  {
    ...
    const pokemon_volatile& tPKV = tTV.teammate(iOTeammates[0]);
    ...
    for (size_t iNTeammate = 0; iNTeammate != cTNV.getNumTeammates(); ++iNTeammate)
    {
      ...
      else
      {
        ...
        const boost::array<uint8_t, 4>& cBestMoves = iBestMoves[iTeam][iTeammate][iOTeammates[0]];
