/**
 * @file legacy_pkCU.h
 * @brief Defines the core battle engine (LegacyPkCU) and its internal state machine
 * (LegacyPkCUEngine).
 *
 * The LegacyPkCU class is the primary interface for simulating Pokemon battles. It
 ...
 *
 * The LegacyPkCUEngine class implements the internal logic of the battle simulation.
 ...
 *
 * This struct is used by the `LegacyPkCUEngine` to store intermediate values during
 ...
 *
 * LegacyPkCU (Pokemon Calculation Unit) is responsible for simulating a single turn
 ...
 */
class PKAISHARED LegacyPkCU {
public:
  /**
   * @struct Config
   * @brief Configuration options for the LegacyPkCU engine.
   */
  struct Config {
    /**
  ...
  LegacyPkCU(const Config& cfg = Config()): cfg_(cfg) {}
  LegacyPkCU(const LegacyPkCU& other) = default;
  ~LegacyPkCU() {};

  LegacyPkCU* clone() const;
   ...
   */
  LegacyPkCU& setEnvironment(const std::shared_ptr<const EnvironmentNonvolatile>& cEnv);
  LegacyPkCU& setEnvironment(const EnvironmentNonvolatile& cEnv) {
 ...
 protected:
   ...
   *
   * The LegacyPkCU instance loads plugins based on the teams in this environment.
   */
 ...
 * the `LegacyPkCU` class. The engine processes a turn by advancing through a series
* of stages, each of which corresponds to a specific part of the turn's
 * mechanics.
 */
class LegacyPkCUEngine {
 protected:
  const LegacyPkCU& cu_;
  const LegacyPkCU::Config& cfg_;
